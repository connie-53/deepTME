...
class Predictor(nn.Module):
    def __init__(self, encoder, decoder, device, atom_dim=1024):
        super().__init__()
...
def pack(atoms, proteins, labels, device):
    ...
    atoms_new = torch.zeros((N, atoms_len, 1024), device=device)
    i = 0
    for atom in atoms:
        a_len = atom.shape[0]
        atoms_new[i, :a_len, :] = atom
        i += 1
    proteins_new = torch.zeros((N, proteins_len, 1024), device=device)
    i = 0
    for protein in proteins:
