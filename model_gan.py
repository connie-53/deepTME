...
class Generator(nn.Module):
    def __init__(self,in_features,out_features=256):
        ...
        """
        super().__init__()
        self.gen = nn.Sequential(nn.Linear(in_features,512)
                                #,nn.BatchNorm1d(256)
                                ,nn.LeakyReLU(0.1)
                                ,nn.Linear(512,out_features)
                                ,nn.Tanh() #用于归一化数据
                                )
        self.disc = nn.Sequential(nn.Linear(256,128),
...
class Encoder(nn.Module):
    ...
    def __init__(self, protein_dim, hid_dim, n_layers, kernel_size, dropout,Generator, device):
        super().__init__()
        ...
        self.Generator = Generator(protein_dim)
    ...
    def forward(self, protein):
        ...
        conv_input=self.Generator(protein)
...
def pack(atoms, proteins, labels, device):
    ...
    for atom in atoms:
        ...
        i += 1
    proteins_new = torch.zeros((N, proteins_len, 34), device=device)
    i = 0
    for protein in proteins:
